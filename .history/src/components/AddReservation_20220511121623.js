import { useState } from "react";
import { addReservation } from "../APIS/Reservation";

export default function AddReservation(){
      let [from , setFrom] = useState("") 
      let [destination , setDestination] = useState("") 


      let [inputs , setInputs] = useState({name: "Akhenaton"} )

      function handleOnChange(event){
          let name = event.target.name
          let value = event.target.value

          setInputs(inputs => ({...inputs , [name]: value}))
      }

     
    let airports = ["Agadir" , "Mohamed V", "Casablanca", "Salé", "Marrakech", "Sidi kasem"]
     
     return (
              <div className="w-50 m-auto mt-5 shadow-sm p-3">
                   <h5>Nouvelle reservation</h5>
                   <div className="form-group mt-4">
                        <label className="form-label">Aéroport départ</label>
                        <select 
                          name = "from"
                          onChange={(event)=> setFrom(event.target.value)}
                          className="form-control">
                            {
                                airports.map((item , index) => <option key={index} value={item}>{item}</option>)
                            }
                        </select>
                   </div>
                   {from}
                   <div className="form-group mt-2">
                        <label className="form-label">Destination</label>
                        <select 
                        name="to"
                         onChange={(event)=> setDestination(event.target.value)}
                        
                        className="form-control">
                            {
                                airports.map((item , index) => <option key={index} value={item}>{item}</option>)
                            }
                        </select>
                   </div>
                   {destination}
                   <div className="form-group mt-2">
                        <label className="form-label">Date départ</label>
                        <input name="date" className="form-control" type="datetime-local"/>
                   </div>
                   <div className="form-group mt-2">
                        <label className="form-label">Nombre de passagers</label>
                        <input name="nbrPassengers" className="form-control" type="number"  min="1" max="3"/>
                   </div>

                   <div className="form-group mt-2">
                        <label className="form-label">Class</label>
                        <select name = "classType" className="form-control">
                           <option value="1">1</option>
                           <option value="2">2</option>
                           <option value="3">3</option>
                        </select>
                   </div>


                   {
                       JSON.stringify(inputs)
                   }

              </div>
    )
}